...
async def help(message: Message):
    await message.answer(
        "Этот бот умеет выполнять команды:\n/start\n/help\n\nТакже можно отправить:\n- Фото\n- Текст для перевода и озвучивания (команда /voice или просто текст)")
...
@dp.message(F.photo)
async def react_photo(message: Message):
    reactions = ['Ого, какая фотка!', 'Непонятно, что это такое', 'Не отправляй мне такое больше']
    rand_answ = random.choice(reactions)
    await message.answer(rand_answ)
    await bot.download(message.photo[-1], destination=f'tmp/{message.photo[-1].file_id}.jpg')


@dp.message(Command('voice'))
async def translate_and_voice_command(message: Message):
    """Обработка команды /voice с текстом"""
    text_to_translate = message.text.replace('/voice', '').strip()

    if not text_to_translate:
        await message.answer("Пожалуйста, напишите текст после команды /voice\nПример: /voice Привет, как дела?")
        return

    await process_translation_and_voice(message, text_to_translate)


@dp.message(F.text & ~F.command)
async def translate_and_voice_text(message: Message):
    """Обработка обычного текстового сообщения"""
    text_to_translate = message.text.strip()
    await process_translation_and_voice(message, text_to_translate)


async def process_translation_and_voice(message: Message, text: str):
    """Основная функция перевода и озвучивания"""
    try:
        # Шаг 1: Переводим текст на английский
        translated_text = GoogleTranslator(source='auto', target='en').translate(text)

        # Шаг 2: Создаем аудиофайл с озвучкой
        ...
        audio_filename = f"tmp/voice_{message.message_id}.mp3"

        # Сохраняем аудиофайл
        tts.save(audio_filename)

        # Шаг 3: Отправляем результаты пользователю
        await message.answer(f"📝 **Оригинальный текст:**\n{text}\n\n"
                             f"🌐 **Перевод на английский:**\n{translated_text}")

        # Отправляем аудиофайл - используем открытый файл
        with open(audio_filename, 'rb') as audio:
            # Отправляем как голосовое сообщение
            await bot.send_voice(
                chat_id=message.chat.id,
                voice=InputFile(audio, filename="translation.mp3"),
